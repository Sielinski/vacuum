...
Our first selection criterion is *z~i~* ≥ *B* · *z̍*. In his example, Tukey sets *B* = 1.5, so our threshold of interest is 1.5*z̍*, indicated by the blue horizontal line. Any point above that line (the shaded blue region) will “deserve special attention”. We have only one such point, colored red. 
...
$$
\Delta x_{jk}=\ z_{jk} \cdot a_{i|n}\cdot\frac{r\cdot c}{\left(r-1\right)\left(c-1\right)}
$$
...
* **Table 8** is a 36 x 15 two-way table and contains example data for the vacuum cleaner (p 54). Note that Tukey's published table contains a typo, which has been corrected in this package (see below).

 "The Future of Data Analysis" contains a handful of errors that can make a detailed investigation of the paper difficult. Below, I've documented the ones that I found in case anyone wants to dig more deeply themselves:
...
* The numerator for [y/ab], when based upon the row coefficient, should be Σ*~r~* *a~r~*[*y/**b***]*~r~* instead of Σ*~r~*  *a~r~*[*y/**a***]*~r~* (p 52)
*	Tukey swaps two values when he plugs them into a key regression equation: 0.258 and 0.167 (p 55). The equation should read:
